...
def restrict_commands(message):
    ...
    if not user_started.get(user_id) or not user_joined_channels.get(user_id):
        ...
        if text.startswith("/download"):
            ...
            bot.reply_to(message, "Invalid command. Please use '/download' or '/audio' followed by a URL.")
...
def download_video(message, url, audio=False, format_id="mp4"):
    ...
    if url_info.scheme:
        ...
        with yt_dlp.YoutubeDL({'cookiefile': 'cookies.txt','ffmpeg_location': 'C:\\ffmpeg\\ffmpeg.exe' , 'format': format_id, 'outtmpl': 'outputs/%(title)s.%(ext)s', 'progress_hooks': [progress], 'postprocessors': [{  # Extract audio using ffmpeg
